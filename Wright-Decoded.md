...
However these are old transactions that use pay-to-public-key as opposed to pay-to-public-key-hash. The public key that Dr. Wright offered is actually exposed in `transaction.outputs.output.script` above (and incidentally transaction `828ef...` also above). So everything he offered as proof of being Satoshi is actually public information.